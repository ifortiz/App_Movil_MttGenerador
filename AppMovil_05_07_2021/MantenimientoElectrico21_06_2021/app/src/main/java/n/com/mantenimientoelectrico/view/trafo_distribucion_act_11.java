...
public class trafo_distribucion_act_11 extends Fragment implements EquipoPruebaListener  {
    ...
    private int numActividad=11;
    List<DetalleActividad> detalleActividades;
    EditText valor_ResisElectricaTotal;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        return inflater.inflate(R.layout.trafo_distribucion_act_11, container, false);
    }
    @Override
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        Button btn_validar = view.findViewById(R.id.btn_save);
        Spinner spinner_equipos=view.findViewById(R.id.spinner1);
        valor_ResisElectricaTotal= view.findViewById(R.id.valor_ResisElectricaTotal);
        ...
        txt_nombreActividad=view.findViewById(R.id.textDeDescricion);
        ...
        btn_atras1.setOnClickListener(v -> Navigation.findNavController(v).navigate(R.id.trafo_distribucion_act_10));
        ...
        actividadModel.getActividadObser().observe(getViewLifecycleOwner(), actividad -> {
            if ( actividad != null ){
                ...
                Navigation.findNavController(requireView()).navigate(R.id.trafo_distribucion_act_12);
            }
        });
    ...
    private void validar() {
        if( valor_ResisElectricaTotal.getText().toString().trim().isEmpty())
            Toast.makeText(getContext(),"No puede dejar campos vacios",Toast.LENGTH_LONG).show();
        else{
            detalleActividades.add( new DetalleActividad( "Resistencia total (Ω)",valor_ResisElectricaTotal.getText().toString()));
            isSave=true;
            Util.showAlertDialogWithOnlyOk("actividades verificadas","Las actividades han sido verificadas",requireContext());
        }
    }
